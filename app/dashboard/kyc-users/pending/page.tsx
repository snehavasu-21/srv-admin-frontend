...
import { Search, Eye, ChevronLeft, ChevronRight, Filter, Clock, Users, AlertCircle, CheckCircle, XCircle, FileText, Image } from "lucide-react";
...
function TypeBadge({ type }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold border ${map[type] ?? "bg-slate-50 text-slate-600 border-slate-200"}`}>
      {type}
    </span>
  );
}

function ImageCell({ label }) {
  return (
    <button
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50 transition-colors duration-150">
                  ...
                  <td className="px-4 py-4 text-sm text-slate-500 max-w-[160px] truncate">
                    {user.address}
                  </td>

                  <td className="px-4 py-4">
                    <ImageCell label="Aadhaar Front" />
                  </td>

                  <td className="px-4 py-4">
                    <ImageCell label="Aadhaar Back" />
                  </td>

                  <td className="px-4 py-4">
                    <ImageCell label="PAN Card" />
                  </td>

                  <td className="px-4 py-4 text-sm text-slate-600 whitespace-nowrap font-mono text-xs">
                    {user.gstNo}
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        className="w-8 h-8 flex items-center justify-center rounded-lg text-green-500 hover:bg-green-50 transition-all duration-200"
                        title="Approve KYC"
                      >
                        <CheckCircle size={15} />
                      </button>
                      <button
                        className="w-8 h-8 flex items-center justify-center rounded-lg text-red-400 hover:bg-red-50 transition-all duration-200"
                        title="Reject KYC"
                      >
                        <XCircle size={15} />
                      </button>
